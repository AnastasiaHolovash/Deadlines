...
//            sv.heightAnchor.constraint(equalTo: horizontalScrollView.heightAnchor - tableView.heightAnchor, multiplier: 1.0)
    ...
    func createHorisontalStackViewWithDates(calendar: Calendar,theLastDate: Date) -> UIStackView{
        ...
        tableViewWidth.constant = CGFloat(screenWigth)
        //tableView.widthAnchor.constraint(equalTo: stackView, multiplier: 1.0).active = true
//NSLayoutConstraint.activate([sv.widthAnchor.constraint(equalToConstant: 55)])
        
        //NSLayoutConstraint.activate([tableView.widthAnchor.constraint(equalToConstant: screenWigth)])
...
//        cell.textLabel?.text = arrayOfNames[indexPath.row]
        ...
        cell.viewWidth.constant = CGFloat((arrayOfDurationOfDeadlines[indexPath.row] - 1) * 65 + 60)
        return cell
    }
    ...
    func getData() {
        ...
        ref.child("\(uid ?? "")/").observe(.value){ (snapshot) in
            ...
            for oneDeadline in dataFromDatabase{
                ...
                numberOfDays = (Calendar.current.dateComponents([.day], from: Date(), to: oneDateOfDeadline).day ?? 0) + 2
